...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.orinal_volume = self.volume
    ...
    def append_string(self, additional_contents):
        ...
        for k in range(len(additional_contents)):
            if len(self.contents) < self.volume:
                ...
                count = count + 1
        leftovers = ''
        for k in range(len(additional_contents) - count):
            leftovers = leftovers + additional_contents[count + k]
        return leftovers
    ...
    def double(self):
        ...
        doubled_thing = self.append_string(self.contents)
        return doubled_thing
    ...
    def shrink(self, new_volume):
        ...
        leftovers = ""
        num_of_leftovers = len(self.contents) - new_volume
        for k in range(new_volume, new_volume + num_of_leftovers):
            leftovers = leftovers + self.contents[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        thing = self.double()
        new = self.shrink(new_volume)
        return len(new) + len(thing)
    ...
    def reset(self):
        ...
        self.contents_backup = self.contents_backup + [self.contents]
        self.volume = self.orinal_volume
    ...
    def steal(self, other_box):
        ...
        extras = self.append_string(other_box.contents)
        other_box.contents = extras
